...
class Critic(nn.Module):
    ...
    def saveCheckpoint(self, ajan, model_name):
        checkpoint_path = os.path.join(model_name, f"{ajan}{self.name}.pth")
        os.makedirs(model_name, exist_ok=True)
        torch.save(self.state_dict(), checkpoint_path)

    def loadCheckpoint(self, ajan, model_name):
        print(f"[Critic] Loading checkpoint - ajan: {ajan}, model_name: {model_name}, self.name: {self.name}")

        # Build possible file paths
        possible_paths = [
            os.path.join(model_name, f"{ajan}{self.name}.pth"),
            os.path.join(model_name, f"{ajan}{self.name}.pt"),
            os.path.join(model_name, f"{ajan}{self.name}"),
            # Legacy path format for backward compatibility
            model_name + '\\{}'.format(ajan) + self.name + '.pth',
            model_name + '\\{}'.format(ajan) + self.name + '.pt',
            model_name + '\\{}'.format(ajan) + self.name,
        ]

        print(f"[Critic] Trying paths: {possible_paths}")

        for path in possible_paths:
            if os.path.isfile(path):
                print(f"[Critic] Found file: {path}")
                try:
                    self.load_state_dict(torch.load(path, map_location=self.device))
                    print(f"[Critic] Successfully loaded: {path}")
                    return
                except Exception as e:
                    print(f"[Critic] Failed to load {path}: {e}")
                    continue

        # Debug: list available files
        if os.path.exists(model_name) and os.path.isdir(model_name):
            files = os.listdir(model_name)
            print(f"[Critic] Available files in {model_name}: {files}")

        raise FileNotFoundError(f"[Critic] Model file not found. Tried: {possible_paths}")
...
class Actor(nn.Module):
    ...
    def loadCheckpoint(self, ajan, model_name):
        print(f"[Actor] Loading checkpoint - ajan: {ajan}, model_name: {model_name}, self.name: {self.name}")
        ...
        print(f"[Actor] Trying paths: {possible_paths}")

        for path in possible_paths:
            if os.path.isfile(path):
                print(f"[Actor] Found file: {path}")
                try:
                    self.load_state_dict(torch.load(path, map_location=self.device))
                    print(f"[Actor] Successfully loaded: {path}")
                    return
                except Exception as e:
                    print(f"[Actor] Failed to load {path}: {e}")
                    continue

        # Debug: list available files
        if os.path.exists(model_name) and os.path.isdir(model_name):
            files = os.listdir(model_name)
            print(f"[Actor] Available files in {model_name}: {files}")

        raise FileNotFoundError(f"[Actor] Model file not found. Tried: {possible_paths}")
...
class Agent(nn.Module):
    def __init__(self, actorLR, criticLR, stateDim, actionDim, full1Dim, full2Dim, tau, gamma, bufferSize, batchSize, \
        ...
        self.targetActor = Actor(actorLR, stateDim, actionDim, full1Dim, full2Dim, layerNorm, 'TargetActor_' + name).to(
            self.device)
        hard_update(self.targetActor, self.actor)

        self.critic = Critic(criticLR, stateDim, actionDim, full1Dim, full2Dim, layerNorm, 'Critic_' + name).to(
            self.device)
        self.targetCritic = Critic(criticLR, stateDim, actionDim, full1Dim, full2Dim, layerNorm,
                                   'TargetCritic_' + name).to(self.device)
        ...
        self.buffer_upsample = True  # train buffer stepsuccess upsample
        self.expert_upsample = True  # bc fire upsample

        print(
            f"buffer upsample is {self.buffer_upsample}, expert upsample is {self.expert_upsample}, loss lambda is {self.loss_lambda}")
    ...
    def chooseAction(self, state):
        ...
        action = (self.actor(state) + \
                  (torch.normal(mean=torch.zeros(self.actionDim),
                                std=torch.ones(self.actionDim) * self.actionNoise)).to(self.actor.device)) \
            .clamp(-1, +1)
    ...
    def chooseActionSmallNoise(self, state):
        ...
        action = (self.actor(state) + \
                  (torch.normal(mean=torch.zeros(self.actionDim),
                                std=torch.ones(self.actionDim) * self.actionNoise / 10)).to(self.actor.device)) \
            .clamp(-1, +1)
        return action.cpu().detach().numpy()
    ...
    def learn(self, bc_weight_now, expert_num_now, bc_warm_up_weight=0):
        ...
        if self.expert_warm_up and expert_num_now:
            ...
            batchState1, batchAction1, batchNextState1, batchReward1, batchDone1 = self.buffer.sample(
                self.batchSize - expert_num_now)
            batchState2, batchAction2, batchNextState2, batchReward2, batchDone2 = self.expert_buffer.sample(
                expert_num_now)
            ...
            batchNextState = torch.tensor(np.array(batchNextState1 + batchNextState2), dtype=torch.float).to(
                self.critic.device)
        ...
        noise = (
            torch.normal(mean=torch.zeros(self.actionDim), std=torch.ones(self.actionDim) * self.TD3LearningNoise)).to(
            self.actor.device)
    ...
    def load_bc_actor(self, ajan, model_name):
        print(f"[Agent] Loading BC actor - ajan: {ajan}, model_name: {model_name}")

        # Try different file path combinations
        possible_paths = [
            # Standard paths with different extensions
            os.path.join(model_name, f"{ajan}.pth"),
            os.path.join(model_name, f"{ajan}.pt"),
            os.path.join(model_name, ajan),
            # Paths with Harfang_GYM suffix
            os.path.join(model_name, f"{ajan}Harfang_GYM.pth"),
            os.path.join(model_name, f"{ajan}Harfang_GYM.pt"),
            os.path.join(model_name, f"{ajan}Harfang_GYM"),
            # Legacy format compatibility
            model_name + '\\{}'.format(ajan) + '.pth',
            model_name + '\\{}'.format(ajan) + '.pt',
            model_name + '\\{}'.format(ajan),
        ]

        print(f"[Agent] Trying BC actor paths: {possible_paths}")

        for path in possible_paths:
            if os.path.isfile(path):
                print(f"[Agent] Found BC actor file: {path}")
                try:
                    state_dict = torch.load(path, map_location=self.device)
                    self.bc_actor.load_state_dict(state_dict)
                    self.bc_actor.eval()
                    print(f"[Agent] Successfully loaded BC actor from: {path}")
                    return
                except Exception as e:
                    print(f"[Agent] Failed to load BC actor from {path}: {e}")
                    continue

        # Debug: list available files
        if os.path.exists(model_name) and os.path.isdir(model_name):
            files = os.listdir(model_name)
            print(f"[Agent] Available files in {model_name}: {files}")

        raise FileNotFoundError(f"[Agent] BC actor model file not found. Tried: {possible_paths}")
