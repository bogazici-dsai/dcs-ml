...
Transition = namedtuple('Transition',
                        ('state', 'action', 'next_state', 'reward', 'done', 'step_success'))

class UniformMemory(object):

    def __init__(self, capacity, upsample):
        self.capacity = capacity
        self.memory = []
        self.position = 0
        self.upsample = upsample
        self.priorities_memory = np.zeros(capacity) if upsample else None
    ...
    def store(self, *args):
        if self.upsample:
            ...
            self.priorities_memory[self.position] = priority
    ...
    def sample(self, batchSize):
        if self.upsample:
            priorities = self.priorities_memory[:len(self.memory)]
