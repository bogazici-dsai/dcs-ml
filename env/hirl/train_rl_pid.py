...
def main(config):
    # GPU print is not needed for PID
    ...
    log_dir = local_config["experiment"][
                  "result_dir"] + "/" + env_type + "/" + agent_name + "/" + (model_name or "PID") + "/" + str(
    ...
    if agent_name == 'TD3':
        ...
        print('Agent: Rule-based PID')
        agent = RuleAgent()
    else:
        raise ValueError(f"Unknown agent type: {agent_name}. Please use 'TD3', 'SAC', or 'PID'.")
    ...
    wandb_instance = setup_wandb(wandb_config, agent_name, env_type, model_name or "PID")
    ...
    # Use SummaryWriter for logging (even for PID agent)
    ...
    for episode in range(trainingEpisodes):
        ...
        if episode % 10 == 0 or episode < 20:
            print(
                f"Episode {episode + 1}: Reward={totalReward:.1f}, MinDist={min_distance:.1f}, Fires={episode_fire_attempts}, Success={episode_successful_fires}, Rate={fire_rate:.3f}, Locks={episode_locks}, Done={done}, Victory={env.episode_success}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--agent', type=str, default='TD3', choices=['TD3', 'SAC', 'PID'],
                        help="Specify the agent to use: 'TD3', 'SAC', or 'PID'. Default is 'TD3'.")
