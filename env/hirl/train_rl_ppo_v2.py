from agents.PPO_v2 import PPOAgentSB3
from utils.seed import *
from environments.HarfangEnv_GYM_ppo_v2 import *
...
def main(config):
    ...
    # IMPROVED ENVIRONMENT SETUP WITH OPTIMIZED HYPERPARAMS
    if env_type == "straight_line":
        ...
        # Optimized hyperparameters for straight line (simplest)
        ent_coef = 0.02        # Increased for better exploration
        n_steps = 512           # Keep current
        learning_rate = 1e-4    # More conservative
        clip_range = 0.2        # Standard value
        batch_size = 128        # Fixed proper size
        ...
        # Optimized hyperparameters for serpentine (medium complexity)
        ent_coef = 0.012        # Higher entropy for more exploration
        n_steps = 1024          # Keep current
        learning_rate = 2e-4    # Even more conservative
        clip_range = 0.2        # Standard value
        batch_size = 256        # Proper size for rollout length
        ...
        # Optimized hyperparameters for circular (most complex)
        ent_coef = 0.015        # Highest entropy for complex environment
        n_steps = 2048          # Keep current
        learning_rate = 1e-4    # Most conservative
        clip_range = 0.15       # Slightly tighter for stability
        batch_size = 512        # Proper size for long rollouts
        n_epochs = 4            # Reduced to prevent overfitting
    ...
    criticLR = 1e-3
    tau = 0.005
    batchSize = batch_size  # Use optimized batch size
    stateDim = 13
    actionDim = 4
    ...
    print('Agent: PPO (Stable-Baselines3) - Optimized Configuration')
    agent = PPOAgentSB3(
        env,
        ...
        vf_coef=0.5,
        max_grad_norm=0.5,
        batch_size=batch_size,      # Pass optimized batch size
        n_epochs=n_epochs,          # Pass optimized epochs
        use_vecnormalize=True,      # Enable normalization
        norm_obs=True,              # Normalize observations
        norm_reward=True            # Normalize rewards
    )
    print(f"[DATA] Optimized PPO Hyperparameters:")
    ...
    wandb_config = {
        ...
        'batchSize': batch_size,        # Use optimized value
        'n_epochs': n_epochs,           # Add to config
        'gamma': gamma,
        'tau': tau,
        'ent_coef': ent_coef,
        'n_steps': n_steps,
        ...
        'vecnormalize_enabled': True,   # Track this setting
    }
    wandb_instance = setup_wandb(wandb_config, agent_name, env_type, model_name or "PPO")

    save_parameters_to_txt(
        log_dir=log_dir, bufferSize=bufferSize, criticLR=criticLR,
        batchSize=batch_size, n_epochs=n_epochs,  # Save optimized values
    ...
    print(f"\n=== PPO TRAINING PHASE (Optimized Configuration) ===")
    ...
    if wandb_instance:
        wandb_instance.log({
            ...
            "setup/entropy_coef": ent_coef,
            "setup/optimization_applied": True,  # Track that optimizations are applied
        })
