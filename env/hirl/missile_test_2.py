...
# TODO: calculate the enemy missiles relative bearing and test. if the missile is behind us, the rb must give values like 170-180
# TODO: add important missile features to the state vector, [missile1, missile2, missile3]
# TODO: focus on enemy missiles with their correct names
# TODO: implement a "missile submodule" in the Harfang environment. it must handle actively tracking enemy missiles and vectorize them, if the missiles are wrecked, it must delete them
# TODO: cont'd, from the state
while True:
	# time.sleep(0.05)
	missile_state = df.get_missile_state("ally_1-AIM_SL-0")
	x, y, z = missile_state["position"][0], missile_state["position"][1], missile_state["position"][2]
	missile_heading = missile_state["heading"]
	print(f"missile positions: {x} {y} {z}  missile heading: {missile_heading}")
