...
def log_to_wandb(wandb_instance, episode, result_dict, phase="training"):
    ...
    try:
        log_dict = {
            ...
            "fire_rate": result_dict.get('fire_rate', 0),
        }
...
def main(config):
    ...
    agent = SACAgent(observation_space=state_space, action_space=action_space, log_dir=log_dir, batch_size=batchSize, lr=actorLR, hidden_units = [hiddenLayer1, hiddenLayer2], memory_size=bufferSize, gamma=gamma, tau=tau)
    writer = agent.writer
    wandb_config = {
    ...
    wandb_instance = setup_wandb(wandb_config, "SAC", env_type, model_name)
    save_parameters_to_txt(log_dir=log_dir, bufferSize=bufferSize, criticLR=criticLR, actorLR=actorLR, batchSize=batchSize, maxStep=maxStep, validationStep=validationStep, hiddenLayer1=hiddenLayer1, hiddenLayer2=hiddenLayer2, agent="SAC", model_dir=agent.model_dir, data_dir=data_dir)
    ...
    if sac_type == 'SAC':
        ...
        for episode in range(explorationEpisodes):
            ...
            if episode % 10 == 0:
                print(f"Exploration {episode + 1}/{explorationEpisodes}: Fire attempts={episode_fire_attempts}, Successful={episode_successful_fires}, Locks={episode_locks}")
        print(f"Exploration Summary: Total fires={exploration_fire_attempts}, Successful={exploration_successful_fires}, Total locks={exploration_locks}")
        print(f"Fire success rate during exploration: {exploration_successful_fires / max(exploration_fire_attempts, 1):.3f}")
        ...
        for episode in range(trainingEpisodes):
            ...
            if wandb_instance:
                wandb_instance.log({
                    ...
                    "training/success_rate_last_50": np.mean(trainsuccess[-50:]) if len(trainsuccess) >= 50 else np.mean(trainsuccess),
                    "training/fire_success_rate_last_50": np.mean(firesuccess[-50:]) if len(firesuccess) >= 50 else np.mean(firesuccess),
                })
            ...
            if (((episode + 1) % checkpointRate) == 0):
                highScore, successRate = validate(
                    validationEpisodes, env, validationStep, agent, plot, agent.plot_dir, arttir, agent.model_dir,
                    episode, checkpointRate, writer, highScore, successRate, if_random, wandb_instance)
                arttir += 1
