...
from harfang_assistant_new.harfang_agent_prompt import render_harfang_prompt
...
class HarfangAssistant:
    ...
    def extract_features(self, state: np.ndarray, prev_state: Optional[np.ndarray], action: np.ndarray,
        ...
        # HarfangEnv_GYM.py packs observations roughly as:
        # Pos_Diff (dx, dy, dz), Plane_Euler (3), target_angle, locked, missile1_state,
        # Oppo_Euler (3), oppo_health, Plane_Pos (3), Oppo_Pos (3), heading, Plane_state (opaque tail)
        # We defensively slice only if available; otherwise default to NaNs/zeros.
        ...
        # Plane and enemy absolute positions (if present)
        ...
        # Closure rate approximate (delta_distance per step)
        closure_rate = float(delta_distance)

        # Aspect angle estimate: use enemy heading vector and relative position if heading present
        try:
            enemy_heading_deg = float(enemy_euler[1]) * 180.0  # rough proxy if heading not explicit
        ...
        # Altitude from plane_pos.y if present
        ...
        # Engagement range categories (example bands)
    ...
    def request_shaping(self, features: Dict[str, Any]) -> Tuple[float, Dict[str, Any]]:
        # Simple rate limiting: reuse last response if calls exceed rate
        now = time.time()
